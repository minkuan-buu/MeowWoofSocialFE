...
export const LikeButton: React.FC<LikeButtonProps> = ({
  ...
  const [showEmotions, setShowEmotions] = useState<{ [key: string]: boolean }>(
    {}
  ); // Trạng thái cho từng bài viết
  ...
  const likeButtonClick = async (postId: string) => {
    if (emojiPost[postId] == null) {
      ...
      var userReact: FeelingGUI = {
        ...
        name: reactionEmojisString[reactionEmojis.indexOf(reactionEmojis[0])],
      };
      setEmojiPost((prev) => ({
        ...prev,
  ...
  const onEmojiClick = async (postId: string, emojiData: EmojiClickData) => {
    ...
    var userReact: FeelingGUI = {
      ...
      name: reactionEmojisString[reactionEmojis.indexOf(emojiData.unified)],
    };
    setEmojiPost((prev) => ({
      ...prev,
      [postId]: userReact, // Ensure value is either string or null
...
export const LikeButtonUserProfile: React.FC<LikeButtonProps> = ({
  postId,
  emojiPost,
  setEmojiPost,
}) => {
  ...
  return (
    <>
      <div className="relative">
        <div
          className={`flex flex-row items-center py-1 ${emojiPost[postId] ? (emojiPost[postId].name.split(" ").length > 1 ? "px-14" : "px-[67px]") : "px-[70px]"} hover:bg-[#e5dfca] hover:text-[#102530] hover:rounded-md transition-all hover-div`}
          onMouseEnter={() => handleMouseEnterLike(postId)}
          onMouseLeave={() => handleMouseLeaveLike(postId)}
          onMouseDownCapture={() => likeButtonClick(postId)}
        >
...
export const LikeButtonImageView: React.FC<LikeButtonProps> = ({
  postId,
  emojiPost,
  setEmojiPost,
}) => {
  ...
  return (
    <>
      <div className="relative">
        <div
          className={`flex flex-row items-center py-1 ${emojiPost[postId] ? (emojiPost[postId].name.split(" ").length > 1 ? "px-4" : "px-5") : "px-6"} hover:bg-[#e5dfca] hover:text-[#102530] hover:rounded-md transition-all hover-div`}
          onMouseEnter={() => handleMouseEnterLike(postId)}
          onMouseLeave={() => handleMouseLeaveLike(postId)}
          onMouseDownCapture={() => likeButtonClick(postId)}
        >
