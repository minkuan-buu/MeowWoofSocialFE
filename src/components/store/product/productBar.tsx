...
export const ProductBar: React.FC<ProductBarProps> = ({ product }) =>  {
  const [selected, setSelected] = useState<string>(product.petStoreProductItems[0].id);
      ...
      <CardBody>
        <div className="grid grid-cols-12">
          <div className="col-span-4">
            <img src={product.attachments.length > 0 ? product.attachments[0].attachment : "/food.png"} alt="product" className="w-[400px] h-[450px] rounded-md" />
          </div>
          <div className="col-span-8 ml-3 flex flex-col justify-between h-full">
            <div>
                ...
                <span className="text-3xl ml-1">
                    ...
                    .find((item) => item.id === selected)?.price
                    .toLocaleString()
                    .replace(",", ".")}
                </span>
                </div>
                ...
                {product.petStoreProductItems.map((item, index) => (
                    <CustomRadio key={index} value={item.id}>{item.name}</CustomRadio>
                ))}
                </RadioGroup>
            </div>
            <div>
                ...
                <div className="flex flex-row items-center mt-5">
                    ...
                    <Button className="!w-12 !h-10 !min-w-0 !p-0 !rounded-full flex items-center justify-center" onClick={() => handleDecrease()}>-</Button>
                    <input 
                        type="text"
                    ...
                    <Button className="!w-12 !h-10 !min-w-0 !p-0 !rounded-full flex items-center justify-center" onClick={() => handleIncrease()}>+</Button>
                    </div>
                </div>
            </div>
            <div className="mt-auto">
                <Button className="border-[#ed5c02]" variant="bordered" size="lg" startContent={<FaCartPlus />}>Thêm vào giỏ hàng</Button>
                <Button className="ml-4 bg-[#ed5c02]" size="lg">Mua ngay</Button>
            </div>
            </div>
        </div>
      </CardBody>
